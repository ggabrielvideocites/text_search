...
class TextIndexDocuments(TextIndexDocumentsABCMeta):
    ...
    def __init__(self, connect_url, separate_string=" ", doc_type="doc"):
        self._es = Elasticsearch(connect_url)
        self._separate_string = separate_string
        self._doc_type = doc_type

    def _get_text_from_field(self, json_or_list, split_by_point_list, place):
        if place >= len(split_by_point_list):
            return json_or_list
        praised_text_field = ""
        if isinstance(json_or_list, list):
            for member in json_or_list:
                praised_text_field += self._separate_string + self._get_text_from_field(json_or_list=member,
    ...
    def _get_text_from_document_fields(self, document, search_field_list):
        ...
        for field in search_field_list:
            ...
            praised_text += self._separate_string + self._get_text_from_field(
    ...
    def _insert_document(self, object_type, document_to_add, tmp_i):
        ...
        #  documents?
        index_in = "{}_index".format(object_type)
        res = self._es.index(index=index_in, doc_type=self._doc_type,id=tmp_i, body=document_to_add)
